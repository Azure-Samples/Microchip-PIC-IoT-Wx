...
void MQTT_CLIENT_iothub_publish(uint8_t* data, uint16_t len)
{
	...
	{
		debug_printError("IOTHUB: az_iot_hub_client_properties_init failed, return code %d\n", rc);
	}
	else if (az_result_failed(rc = az_iot_message_properties_append(&properties_topic, AZ_SPAN_FROM_STR("foo"), AZ_SPAN_FROM_STR("bar"))))
	{
		debug_printError("IOTHUB: az_iot_hub_client_properties_append failed, return code %d\n", rc);
	}
	else if (az_result_failed(rc = az_iot_hub_client_telemetry_get_publish_topic(
